...
class GoMerchant {
    constructor() {
        ...
        this.uniqueId = crypto.randomUUID ? crypto.randomUUID() : 'c9b4e789-509a-4c9f-b328-98449c25227d';
    }
    ...
    async createDynamicQRIS(amount, staticQr) {
        try {
            ...
            if (step2.length < 2) {
                throw new Error("Format static QRIS tidak valid. Pastikan QRIS mengandung tag '5802ID'.");
            }
    ...
    async getJournals(accessToken, merchantId, startTime = null) {
        ...
        const dateFrom = startTime ? new Date(startTime).toISOString() : new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString();
        const payload = {
            from: 0,
            size: 50,
    ...
    async cancelTransaction(token, transactionId) {
        const response = await axios.post(`${this.baseUrl}/v1/merchants/transactions/${transactionId}/cancel`, {}, {
            headers: this.headers(token)
        });
        return response.data;
    }

    async getTransactionDetail(token, transactionId) {
        const response = await axios.get(`${this.baseUrl}/v1/merchants/transactions/${transactionId}`, {
            headers: this.headers(token)
        });
        return response.data;
    }
